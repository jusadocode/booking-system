...
import { Container, Table, CircularProgress, TableBody, TableCell, TableContainer, Button, TableHead, TableRow, Paper, Typography, Collapse, IconButton, Box, TextField } from '@mui/material';
                ...
                <TableBody>
                    {filteredHotels.map((hotel) => (
                        <React.Fragment key={hotel.id}>
                            <TableRow className='hotelTableRow' onClick={() => handleExpandClick(hotel.id)} style={{ cursor: 'pointer' }}>
                                <TableCell>
                                    <img src={hotel.imageUrl} alt={hotel.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                                </TableCell>
                                <TableCell>
                                    <Typography variant="body1" noWrap>{hotel.name}</Typography>
                                </TableCell>
                                <TableCell>
                                    <Typography variant="body1" noWrap>{hotel.address}</Typography>
                                </TableCell>
                                <TableCell>
                                    <IconButton
                                                ...
                                                <TableBody>
                                                    {hotel.rooms.map((room) => (
                                                        <TableRow className='roomTableRow' key={room.id}>
                                                            ...
                                                            <TableCell>
                                                                {Array.from({ length: room.numOfGuests }, (_, index) => (
                                                            ...
                                                            <TableCell>
                                                                <Button variant="contained" onClick={() => handleOpenDialog(room)}>Book</Button>
                                                            </TableCell>
                                                        </TableRow>
                                                    ))}
                                                </TableBody>
            ...
            <Container style={{ marginTop: '20px' }}>
                ...
                {hotels.length === 0 ? (
                    <Box display="flex" alignItems="center">
                        <CircularProgress />
                        <Typography style={{ marginLeft: '10px' }}>Getting hotel data...</Typography>
                    </Box>
                ) : (
                    filteredHotels.length > 0 ? hotelList : "No hotels found by the location"
                )}
            </Container>
            <BookingDialog
                open={dialogOpen}
                ...
                expandedHotel={filteredHotels.find(hotel => hotel.id === expandedHotel)}
            />
        </>
    );
}
